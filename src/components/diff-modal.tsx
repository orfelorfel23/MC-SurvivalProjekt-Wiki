...
export function DiffModal({ 
  ...
  const getChanges = () => {
    ...
    allKeys.forEach(key => {
      ...
      const oldValStr = typeof oldData[key] === 'object' ? JSON.stringify(oldData[key]) : String(oldData[key] ?? "");
      const newValStr = typeof newData[key] === 'object' ? JSON.stringify(newData[key]) : String(newData[key] ?? "");
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="max-w-3xl max-h-[80vh] overflow-y-auto">
        ...
        <DialogFooter>
          ...
          <Button onClick={() => { onConfirm(); onClose(); }}>Änderungen speichern</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
