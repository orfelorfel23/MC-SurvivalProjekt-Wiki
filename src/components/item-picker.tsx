...
export function ItemPicker({ slot, onChange, onClear }: { slot: GridSlot, onChange: (s: GridSlot) => void, onClear: () => void }) {
  ...
  const filteredVanilla = vanilla.filter(i => i.name.toLowerCase().includes(query) || i.id.toLowerCase().includes(query)).slice(0, 50);

  return (
    <Dialog open={open} onOpenChange={(v) => { setOpen(v); if(!v) setSelectedVanilla(null); }}>
      <DialogTrigger asChild>
        <button className="w-12 h-12 border border-border border-dashed flex items-center justify-center hover:bg-accent/10 relative group">
          {slot ? (
             slot.type === "db" ? <div className="text-[10px] truncate max-w-full px-1">DB Item</div> 
             : (
               <img src={`/items/${slot.mc_id}.png`} className={`w-8 h-8 object-contain ${slot.enchanted ? 'mc-glint' : ''}`} alt={slot.name || slot.mc_id} title={slot.name || slot.mc_id} />
             )
        ...
        {selectedVanilla ? (
          <div className="flex flex-col gap-4 py-4 flex-1">
            <div className="flex items-center gap-4">
              <img src={selectedVanilla.url} className={`w-16 h-16 object-contain ${enchanted ? 'mc-glint' : ''}`} />
            ...
            <div className="grid gap-2">
              ...
              <Input value={customName} onChange={e => setCustomName(e.target.value)} placeholder={selectedVanilla.name} />
            </div>
            ...
            <div className="flex gap-2 mt-auto">
              <Button onClick={() => setSelectedVanilla(null)} variant="outline">Zurück</Button>
          ...
          <div className="flex flex-col gap-4 overflow-hidden flex-1">
            <div className="relative">
              ...
              <Input placeholder="Suchen..." value={q} onChange={e => setQ(e.target.value)} className="pl-8" />
            </div>
              ...
              {filteredDb.length > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-4 gap-2">
                    {filteredDb.map(item => (
                      <button key={item.id} className="mc-panel p-2 flex flex-col items-center gap-1 hover:bg-accent/10" onClick={() => {
                        onChange({ type: "db", item_id: item.id });
                        setOpen(false);
                      }}>
                        {item.imageUrl && <img src={item.imageUrl} className={`w-8 h-8 object-contain ${item.enchanted ? 'mc-glint' : ''}`} />}
            ...
            {slot && (
              <Button variant="destructive" onClick={() => { onClear(); setOpen(false); }}>
                Item entfernen
              </Button>
            )}
          </div>
