async function runTests() {
  ...
  await test("Public Pages - Startseite lädt erfolgreich", async () => {
    ...
    if (!text.includes("Server Wiki"))
      throw new Error('Titel "Server Wiki" nicht im HTML gefunden');
  });
  ...
  await test("Suchfunktion - Seite lädt", async () => {
    ...
    if (!res.ok) throw new Error(`HTTP Error ${res.status}`);
    const text = await res.text();
    if (!text.includes("Erweiterte Suche"))
      throw new Error('Titel "Erweiterte Suche" nicht gefunden');
  });
